...
export const PortfolioGraph = () => {
    const {graphData, range, setRange} = usePortfolioGraph()

    // TODO: Loading spinner while waiting for data to load
    return (
        <div className={"rounded-2xl basis-3/4 bg-blue-950 py-4 px-6 self-stretch"}>
            <div className={"flex flex-row justify-end"}>
                ...
                <div className={"basis-2/5 flex flex-row items-center justify-end gap-4"}>
                    <button onClick={() => setRange("week")} className={"btn btn-sm" + (range === "week" ? " border-white" : "") }>Week</button>
                    <button onClick={() => setRange("month")} className={"btn btn-sm" + (range === "month" ? " border-white" : "") }>Month</button>
                    <button onClick={() => setRange("year")} className={"btn btn-sm" + (range === "year" ? " border-white" : "") }>Year</button>
                    <button onClick={() => setRange("max")} className={"btn btn-sm" + (range === "max" ? " border-white" : "") }>Max</button>
                </div>
            </div>

            <div className={"mt-4 h-[475px] rounded-2xl"}>
                <StockGraph stockData={graphData}/>
            </div>
        </div>
    );
};
...
export const StockGraph = ({stockData}: { stockData: GraphAsset[] }) => {
    ...
    return (
        <div ref={ref} className="container h-full">
